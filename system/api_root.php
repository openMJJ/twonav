...
function write_sys_settings(){
    ...
    foreach (['c_name','c_desc','l_name','l_url','l_key','l_desc'] as $name){
		...
		$length_limit[$name] = intval($_POST[$name]) ? intval($_POST[$name]) : 0;
    }
	...
	*/
	if($_POST['Sub_domain'] == 1){$o_config['Sub_domain'] = $_POST['Sub_domain'];}
	if($_POST['Privacy'] == 1){$o_config['Privacy'] = $_POST['Privacy'];}
	if(!empty($_POST['copyright'])){$o_config['copyright'] = "";}
	if(!empty($_POST['global_header'])){$o_config['global_header'] = "";}
	if(!empty($_POST['global_footer'])){$o_config['global_footer'] = "";}
	if($_POST['apply'] == 1){$o_config['apply'] = $_POST['apply'];}
	if($_POST['guestbook'] == 1){$o_config['guestbook'] = $_POST['guestbook'];}
	if($_POST['link_extend'] == 1){$o_config['link_extend'] = $_POST['link_extend'];}
	if($_POST['article'] == 1){$o_config['article'] = $_POST['article'];}
	if($_POST['static_link'] == 1){$o_config['static_link'] = $_POST['static_link'];}
	...
	update_db("global_config", ["v" => $o_config], ["k" => "o_config"],[1,"保存成功"]);
}
