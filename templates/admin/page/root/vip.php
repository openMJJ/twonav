...
layui.use(['jquery','form'], function () {
    ...
    form.on('submit(get_subscribe)', function(data){
        ...
        $.get('https://service.twonav.cn/api.php?fn=get_subscribe',data.field,function(data,status){
    ...
    form.on('submit(set_subscribe)', function(data){
        ...
        layer.load(2, {shade: [0.1,'#fff']});
        $.get('https://service.twonav.cn/api.php?fn=check_subscribe',data.field,function(data,status){
            layer.closeAll('loading');
            if(data.code == 200) {
                $("#end_time").val(timestampToTime(data.data.end_time));
